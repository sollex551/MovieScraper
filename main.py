...
if __name__ == "__main__":
    ...
    for j, film in enumerate(films):
        if film.get("title"):
            ...
            films_dict[index] = film["title"]
            index += 1
    choice = int(input("Введите номер фильма для просмотра: "))
    print(Fore.GREEN + f"Вы выбрали: {films_dict.get(choice, 'Фильм не найден')}")
    name = get_film_title_by_number(films_dict, choice)
    page = None
    for i in films:
        if i.get('title') == get_film_title_by_number(films_dict, choice):
            page = i.get('page')
            break
    else:
    ...
    for j, film in enumerate(filmss):
        if film.get("size"):
            print(Fore.CYAN + f'[{inde}]' + Fore.BLUE + f' {film["size"]}')
    ...
    try:
        download_film(dw, films_dict.get(choice))
    except Exception as e:
        download_film(dw, 'None')
